...
export default function Edit({rl, close, successCallback}) {
    const {authFetch} = useAuth()
    const [role, setRole] = useState(rl)
    ...
    const updateRole = async () => {
        ...
        const response = await authFetch(`/api/roles/${rl.id}`, {
            method: "PUT",
            headers: {
        ...
        <div className={`animate-fadeIn w-full flex flex-row gap-5`}>
            ...
            <div className={`w-full flex flex-col`}>
                ...
                <div className={`mt-5 flex flex-row gap-5`}>
                    ...
                    <button onClick={updateRole} className={`primary-b w-full h-full`}>
                        <Check />
                    </button>
                </div>
